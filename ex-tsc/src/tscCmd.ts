...
export class TscCmd {
  ...
  private _queryPathsConfig(baseUrl: string, pathsJson: object): object {
    ...
    Object.keys(pathsJson).forEach(key => {
      pathsConfig[this._getNoAsteriskStr(key)] = join(baseUrl, this._getNoAsteriskStr(pathsJson[key][0]);
    });
    return pathsConfig;
  }


  private _getNoAsteriskStr(str) {
    const result = str.replace(this.reg, "");
    return result;
  }
